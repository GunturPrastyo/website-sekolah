...
const filteredPrograms = computed(() => {
  ...
  return programList.value.filter(
    (prog) =>
      ...
      prog.badge.toLowerCase().includes(query)
  );
});
</script>
  ...
  <main class="flex-1 overflow-y-auto px-6 md:px-10 py-8">
      ...
      >
        ...
        <form @submit.prevent="isEditing ? saveEntry() : addEntry()">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="md:col-span-1">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Fokus Pembelajaran <span class="text-xs text-gray-500">(Pisahkan dengan koma)</span>
              </label>
              <textarea
            ...
            <div class="md:col-span-1">
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Prospek Karir <span class="text-xs text-gray-500">(Pisahkan dengan koma)</span>
              </label>
              <textarea
    ...
    <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm">
      <div class="mb-6 relative max-w-md">
        ...
        <input type="text" v-model="searchQuery" placeholder="Cari jurusan..." class="block w-full pl-10 pr-3 py-2.5 border border-gray-300 dark:border-slate-600 rounded-lg bg-gray-50 dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
      </div>

      <div v-if="filteredPrograms.length === 0" class="py-12 text-center text-gray-500 dark:text-gray-400 border-2 border-dashed border-gray-200 dark:border-slate-700 rounded-xl">
        <p>Tidak ada data jurusan yang ditemukan.</p>
      </div>

      <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div v-for="prog in filteredPrograms" :key="prog.id" class="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-600 shadow-sm p-5 relative group">
          <div class="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 bg-white/90 dark:bg-slate-800/90 p-1 rounded-lg border border-gray-100 dark:border-slate-700 z-10">
            <button @click="startEdit(prog)" class="p-1.5 text-blue-600 hover:bg-blue-50 dark:hover:bg-slate-700 rounded-md"><PhPencilSimple class="w-4 h-4" /></button>
            <button @click="deleteEntry(prog.id)" class="p-1.5 text-red-600 hover:bg-red-50 dark:hover:bg-slate-700 rounded-md"><PhTrash class="w-4 h-4" /></button>
          </div>
          <div class="flex items-center gap-3 mb-3">
            <div class="w-10 h-10 shrink-0 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-600 flex items-center justify-center">
              <PhGraduationCap class="w-5 h-5" />
            </div>
            <div>
              <h4 class="font-bold text-gray-900 dark:text-white">{{ prog.title }}</h4>
              <p class="text-xs font-medium text-blue-600 dark:text-blue-400">{{ prog.badge }}</p>
            </div>
          </div>
          <p class="text-sm text-gray-600 dark:text-gray-300 line-clamp-3 mb-4">{{ prog.description }}</p>
          <div class="grid grid-cols-2 gap-4 mt-auto border-t border-gray-100 dark:border-slate-700 pt-3">
            <div>
              <p class="text-xs font-semibold text-gray-700 dark:text-gray-300 flex items-center mb-1"><PhBookOpen class="w-3.5 h-3.5 mr-1"/> Topik:</p>
              <p class="text-xs text-gray-500 line-clamp-2">{{ prog.subjects }}</p>
            </div>
            <div>
              <p class="text-xs font-semibold text-gray-700 dark:text-gray-300 flex items-center mb-1"><PhBriefcase class="w-3.5 h-3.5 mr-1"/> Karir:</p>
    ...
    <ConfirmModal :isOpen="isDeleteModalOpen" title="Hapus Data Jurusan" message="Yakin ingin menghapus data ini?" @confirm="confirmDelete" @cancel="cancelDelete" />
