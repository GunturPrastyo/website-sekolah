...
const misi = ref([
  { id: 1, text: "Menumbuhkembangkan keimanan dan ketaqwaan melalui pembiasaan kegiatan keagamaan di lingkungan sekolah." },
  { id: 2, text: "Melaksanakan proses pembelajaran dan bimbingan secara efektif, inovatif, dan berpusat pada peserta didik." },
  { id: 3, text: "Membina peserta didik untuk mengembangkan minat, bakat, dan potensi yang dimiliki melalui kegiatan ekstrakurikuler." },
  { id: 4, text: "Mewujudkan lingkungan sekolah yang bersih, asri, aman, dan nyaman sebagai pilar utama pendukung proses belajar mengajar." },
  { id: 5, text: "Meningkatkan kemampuan pemanfaatan teknologi informasi dan komunikasi untuk siap bersaing di tingkat global." },
]);
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
      ...
      <div class="xl:col-span-2 space-y-8">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-6">
          <h3 class="text-xl font-semibold text-gray-800 dark:text-white mb-4">Visi Sekolah</h3>
          <textarea
            v-model="visi"
            rows="3"
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-6">
          <div class="flex flex-wrap items-center justify-between mb-4 gap-2">
            <h3 class="text-xl font-semibold text-gray-800 dark:text-white">Misi Sekolah</h3>
            <button @click="addMisi" class="inline-flex items-center px-3 py-1.5 text-sm font-medium text-blue-600 bg-blue-50 dark:bg-blue-900/30 dark:text-blue-400 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors">
              <PhPlusCircle class="w-5 h-5 mr-1.5" /> Tambah Misi
            </button>
          </div>
          <div class="space-y-3">
            <div v-for="(item, index) in misi" :key="item.id" class="flex gap-3 items-start group" draggable="true" @dragstart="handleDragStart(index, $event)" @dragover.prevent @dragenter.prevent @drop="handleDrop(index)">
              <div class="mt-2 text-gray-400 cursor-grab active:cursor-grabbing hover:text-gray-600 dark:hover:text-gray-300 shrink-0" title="Tahan dan geser untuk memindahkan">
                <PhDotsSixVertical class="w-6 h-6" />
              </div>
              <div class="flex-1">
                <textarea v-model="item.text" rows="2" class="w-full px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-xl bg-gray-50 dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-blue-500 focus:border-blue-500 resize-none transition-colors text-sm sm:text-base" placeholder="Tuliskan butir misi..."></textarea>
              </div>
              <button @click="removeMisi(index)" class="mt-1 p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors shrink-0" title="Hapus Misi">
                <PhTrash class="w-5 h-5" />
              </button>
            </div>
            <div v-if="misi.length === 0" class="text-center py-6 text-gray-500 dark:text-gray-400 text-sm border-2 border-dashed border-gray-200 dark:border-slate-700 rounded-xl">
      ...
      <div class="xl:col-span-1">
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-6 sticky top-6">
          <h3 class="text-xl font-semibold text-gray-800 dark:text-white mb-4">Sambutan Kepala Sekolah</h3>
          <div class="mb-4">
            <RichTextEditor v-model="sambutan" placeholder="Tuliskan sambutan kepala sekolah..." />
          </div>
          <p class="text-xs text-gray-500 dark:text-gray-400 leading-relaxed mt-4">
            Teks sambutan ini akan tampil pada halaman <strong>"Visi, Misi & Tujuan"</strong> di antarmuka publik website.
          </p>
        </div>
      </div>
    </div>
